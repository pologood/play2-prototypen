...
public class MyDeadboltHandler extends AbstractDeadboltHandler {
	...
	private String[] knownAJAXRoutes = new String[] { 
			controllers.common.routes.Application.secureJsonRedirect().url() 
	};
	...
	public Result beforeAuthCheck(final Http.Context context) {
		if (PlayAuthenticate.isLoggedIn(context.session())) {
			...
			// AJAX redirect preparation
			String origUrl = context.request().path();
			// check if secured AJAX request and put status
			for (String securedAjaxRoute : knownAJAXRoutes) {
				if (origUrl.compareTo(securedAjaxRoute) == 0) {
					ObjectNode result = Json.newObject();
					result.put("status", "Authentification Error");
